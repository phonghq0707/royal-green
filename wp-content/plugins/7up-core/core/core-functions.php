...
function s7upf_scrape_instagram($username, $slice = 9 , $token = '',$size_index= 0) {
    ...
    if($username) {
        ...
        return new WP_Error('site_down', __('Unable to communicate with Instagram.', '7up-core'));
        if ( 200 != wp_remote_retrieve_response_code( $remote ) )
        return new WP_Error('invalid_response', __('Instagram did not return a 200.', '7up-core'));
        $shards = explode('window._sharedData = ', $remote['body']);
        $insta_json = explode(';</script>', $shards[1]);
        $insta_array = json_decode($insta_json[0], TRUE);
        if (!$insta_array)
        return new WP_Error('bad_json', __('Instagram has returned invalid data.', '7up-core'));
        if(!empty($token)){
...
if(!function_exists('s7upf_fix_decodeb64')){
    function s7upf_fix_decodeb64( $data , $strict = FALSE ){
        return base64_decode( $data , $strict );
    }
}
if(!function_exists('s7upf_reg_widget')){
    function s7upf_reg_widget( $widget_class ){
