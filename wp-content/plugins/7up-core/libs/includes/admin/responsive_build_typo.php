...
if ( ! class_exists( 'S7upfResVCBuildTypo' ) ) {
	/**
	 * VC S7upfResVCBuildCSS Class
	 *
	 * @since	1.0
	 */
	class S7upfResVCBuildTypo {
		...
		public function init() {
			$this->shortcodes = S7upfResVCHelper::$shortcodes_active;

			$this->screens = S7upfResVCHelper::$devices_array;
			...
			foreach ( $shortcodes[2] as $index => $tag ) {
				...
				if ( isset( $shortcode['params'] ) && ! empty( $shortcode['params'] ) ) {
					foreach ( $shortcode['params'] as $param ) {
						...
						if ( 's7upf_typography' === $param['type'] && isset( $attr_array[ $param['param_name'] ] ) && array_key_exists($param['param_name'], $this->screens) ) {
