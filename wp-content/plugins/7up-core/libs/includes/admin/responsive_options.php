...
if ( ! class_exists( 'S7upfResVCOptions' ) ) {
	/**
	 * VC S7upfResVCOptions Class
	 *
	 * @since	1.0
	 */
	class S7upfResVCOptions {
		...
		public function init() {
			$this->devices = S7upfResVCHelper::$devices;
		}
		...
		public function admin_menu(){
			...
			add_menu_page(
		        esc_html('Responsive Settings', '7up-core'),
		        esc_html('Responsive Settings', '7up-core'),
		        'manage_options',
		        's7upf_res_options',
			...
			add_submenu_page(
				's7upf_res_options',
				esc_html('General Settings', '7up-core'),
				esc_html('General Settings', '7up-core'),
				'manage_options',
				's7upf_res_options',
				array(&$this, 'generalSettings' )
			...
			add_submenu_page(
				's7upf_res_options',
				esc_html('All Devices', '7up-core'),
				esc_html('All Devices', '7up-core'),
				'manage_options',
				's7upf_res_all_devices',
				array(&$this, 'allDevices' )
			);

			add_submenu_page(
				's7upf_res_options',
				esc_html('Add New Device', '7up-core'),
				esc_html('Add New Device', '7up-core'),
				'manage_options',
				's7upf_res_addnewdevice',
				array(&$this, 'addNewDevice' )
			);

			if( isset( $_POST['s7upf_res_update_general'] ) && ( '1' === $_POST['s7upf_res_update_general'] || 'true' === $_POST['s7upf_res_update_general'] ) ) {
				$this->update_general();
			}

			if( isset( $_POST['s7upf_res_addnewdevice'] ) && ( '1' === $_POST['s7upf_res_addnewdevice'] || 'true' === $_POST['s7upf_res_addnewdevice'] ) ) {
				$this->request_addnewdevice();
			}

			if( isset( $_POST['s7upfResIdDeviceDel'] ) && '' != $_POST['s7upfResIdDeviceDel'] ) {
				$this->delete_device();
			}

			if( isset( $_POST['s7upf_res_update_order'] ) && '1' === $_POST['s7upf_res_update_order'] ) {
			...
			if( isset( $_GET['idEdit'] ) && '' != $_GET['idEdit'] ) {
				...
				if( isset( $_POST['s7upf_res_updatedevice'] ) && ( '1' === $_POST['s7upf_res_updatedevice'] || 'true' === $_POST['s7upf_res_updatedevice'] ) ) {
					$this->update_device();
				}
			}
		...
		public function enqueueScript(){
			wp_enqueue_script( 'bb_vcedo', CORE7UP_PLUGIN_URL . '/libs/assets/admin/js/admin.js', array( 'jquery' ), '1.0', true );
			wp_enqueue_style( 'bb_vcedo', CORE7UP_PLUGIN_URL . '/libs/assets/admin/css/admin.css' );
		}
		...
		public function update_device(){
			...
			if( $this->idEdit != '' && array_key_exists($this->idEdit, $this->devices) ) {
				...
				// 		$this->notice = esc_html__( 'ID must not empty or duplicate!', '7up-core' );
				...
				// 	$this->idEdit = $id;
				// 	$this->devices = S7upfResVCHelper::array_unshift_assoc($this->devices, $id, $dataDevice);
				//
				// }

				$this->sort_devices();

				if( self::update_option(CORE7UP_DEVICES, $this->devices) ) {
					$this->build_css_show_hide();
					$this->notice = esc_html__( 'Updated device!', '7up-core' );
					add_action( 'admin_notices', array( $this, 'update_notice' ) );
				} else {
					$this->notice = esc_html__( 'Irks! An error has occurred.', '7up-core' );
					add_action( 'admin_notices', array( $this, 'error_notice' ) );
				}
		...
		public function delete_device(){
			...
			if($this->s7upfResIdDeviceDel == '' || !array_key_exists($this->s7upfResIdDeviceDel, $this->devices)) {
				$this->notice = esc_html__( 'Not found any Devices have this ID', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
				return;
			}

			unset($this->devices[$this->s7upfResIdDeviceDel]);

			if( self::update_option(CORE7UP_DEVICES, $this->devices) ) {
				$this->resetDeviceDetail();
				$this->notice = esc_html__( 'Deleted device!', '7up-core' );
				add_action( 'admin_notices', array( $this, 'update_notice' ) );
			} else {
				$this->notice = esc_html__( 'Irks! An error has occurred.', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
		...
		public function update_general(){
			...
			if( self::update_option(CORE7UP_OPTIONS_BY_ELEMENTS, $this->option_by_elements) ||
			self::update_option(CORE7UP_CUSTOM_ELEMENTS, $this->custom_elements) ||
			self::update_option(CORE7UP_MENU_TAB_POSITION, $this->menu_tab_position) ) {
				$this->notice = esc_html__( 'Updated', '7up-core' );
				add_action( 'admin_notices', array( $this, 'update_notice' ) );
			} else {
				$this->notice = esc_html__( 'Irks! An error has occurred.', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
		...
		public function update_order_device(){
			...
			if( self::update_option(CORE7UP_DEVICES, $this->devices) ) {
				$this->build_css_show_hide();
				$this->notice = esc_html__( 'Updated devices!', '7up-core' );
				add_action( 'admin_notices', array( $this, 'update_notice' ) );
			} else {
				$this->notice = esc_html__( 'Irks! An error has occurred.', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
		...
		public function request_addnewdevice(){
			...
			if($this->idSlug == '') {
				$this->notice = esc_html__( 'ID must not empty!', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
				return;
			}
			if(array_key_exists($this->idSlug, $this->devices) ) {
				$this->notice = esc_html__( 'ID exists, ID must not duplicate!', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
				return;
			}
			...
			if( self::update_option(CORE7UP_DEVICES, $this->devices) ) {
				$this->resetDeviceDetail();
				$this->build_css_show_hide();
				$this->notice = esc_html__( 'Added device!', '7up-core' );
				add_action( 'admin_notices', array( $this, 'update_notice' ) );
			} else {
				$this->notice = esc_html__( 'Irks! An error has occurred.', '7up-core' );
				add_action( 'admin_notices', array( $this, 'error_notice' ) );
		...
		public function build_css_show_hide(){
			...
			foreach ($this->devices as $id => $device) {
				...
				if(empty($mediafeature) || empty($breakpoint)) {
					$css .= ' .' . CORE7UP_PARAM_PREFIX . $id . '_hide {display:none!important}';
					$css .= ' .' . CORE7UP_PARAM_PREFIX . $id . '_show {display:block!important}';
					continue;
				}
			...
			self::update_option(CORE7UP_SHOWHIDE, $css);
		}
	...
	new S7upfResVCOptions(
		S7upfResVCHelper::$option_by_elements,
		S7upfResVCHelper::$custom_elements,
		S7upfResVCHelper::$menu_tab_position
	);
}
