...
if ( ! class_exists( 'S7upfResVCHelper' ) ) {
	/**
	 * VC S7upfResVCHelper Class
	 *
	 * @since	1.0
	 */
	class S7upfResVCHelper {
		...
		function __construct() {
			...
			self::$tab_option = esc_html('Responsive Design Options', '7up-core');
			self::$typo_label = esc_html('Block Settings', '7up-core');
			self::$tab_active = CORE7UP_PARAM_PREFIX . 'x_large_css';

			self::$option_by_elements = get_option(CORE7UP_OPTIONS_BY_ELEMENTS, apply_filters( 's7upf_options_by_elements_default', '' ) );
			self::$custom_elements = get_option(CORE7UP_CUSTOM_ELEMENTS, apply_filters( 's7upf_custom_elements_default', 'vc_row,vc_column' ) );
			self::$menu_tab_position = get_option(CORE7UP_MENU_TAB_POSITION, apply_filters( 's7upf_menu_tab_position_default', 'right' ) );

			self::$show_hide_css = get_option(CORE7UP_SHOWHIDE, apply_filters( 's7upf_css_show_hide_default', ' .s7upf_x_large_css_hide {display:none!important} .s7upf_x_large_css_show {display:block!important} @media (max-width: 1199px) {  .s7upf_large_css_hide {display:none!important} .s7upf_large_css_show {display:block!important}}  @media (max-width: 991px) {  .s7upf_medium_css_hide {display:none!important} .s7upf_medium_css_show {display:block!important}}  @media (max-width: 767px) {  .s7upf_small_css_hide {display:none!important} .s7upf_small_css_show {display:block!important}} ' ) );
		...
		public function init(){
			...
			self::$devices = get_option(CORE7UP_DEVICES,
			...
			foreach (self::$devices as $id => $device) {
				self::$devices_array[ CORE7UP_PARAM_PREFIX . $id] = $device;
			}
		}
