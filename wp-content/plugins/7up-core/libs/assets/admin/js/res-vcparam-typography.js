(function($) {
	...
	function bb_typo_build_css(){
		var bb_typography_el = $('.s7upf-typography-value');
		...
		bb_typography_el.each(function(index){
			...
			var _self = $(this),
				bb_typo_container = _self.closest('.s7upf-typography');
	...
	$('document').ready(function(){
		$('.hide-color-picker.s7upf-colorpicker-opacity').opacity_wpColorPicker();
		...
		$('.res-title-tab').on('click',function(event){
			...
			$(this).parents('.s7upf-typography').find('.block-wrap-content').fadeOut();
			$(this).parents('.s7upf-typography').find('.block-wrap-content[data-res="'+res_id+'"]').fadeIn();
		})
		$('.s7upf-typography .iris-palette,.wp-picker-clear').on('click', function(){
			setTimeout( bb_typo_build_css, 100);
		});

		$('.s7upf-typography').on('keydown' , 'input', function(){
			setTimeout( bb_typo_build_css, 100);
		});
		
		$('.s7upf-typography').on('change' , 'select', function(){
			setTimeout( bb_typo_build_css, 100);
		});
