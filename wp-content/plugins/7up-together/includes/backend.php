...
function famibt_save_all_settings_via_ajax() {
	...
	if ( ! current_user_can( 'manage_options' ) ) {
		$response['message'][] = esc_html__( 'Cheating!? Huh?', '7up-together' );
		$response['err']       = 'yes';
		wp_send_json( $response );
	}
	
	if ( ! wp_verify_nonce( $nonce, 'famibt_backend_nonce' ) ) {
		$response['message'][] = esc_html__( 'Security check error!', '7up-together' );
		$response['err']       = 'yes';
		wp_send_json( $response );
	}
...
//	$response['new_all_settings'] = $new_all_settings;
	...
	$response['message'][] = esc_html__( 'All settings saved', '7up-together' );
	wp_send_json( $response );
	die();
}
