...
if ( ! class_exists( 'famibtMenuScriptsStyles' ) ) {
	class famibtMenuScriptsStyles {
		...
		public function famibt_admin_bar_menu() {
			...
			$argsParent = array(
				'id'    => 'famibt_option',
				'title' => esc_html__( 'Buy Together', '7up-together' ),
				'href'  => admin_url( 'admin.php?page=s7up_together' ),
			);
		...
		public function famibt_menu_page() {
			...
			$menu_args = array(
				array(
					'page_title' => esc_html__( 'Buy Together', '7up-together' ),
					'menu_title' => esc_html__( 'Buy Together', '7up-together' ),
					'cap'        => 'manage_options',
					'menu_slug'  => 's7up_together',
		...
		function frontend_scripts( $hook ) {
			...
			$famibt_args = array(
				...
				'text'     => array(
					'for_num_of_items'         => esc_html__( 'For {{number}} item(s)', '7up-together' ),
					'add_to_cart_text'         => $add_to_cart_text,
					'adding_to_cart_text'      => $adding_to_cart_text,
					'view_cart'                => $view_cart_text,
					'no_product_selected_text' => $no_product_selected_text,
					'add_to_cart_success'      => esc_html__( '{{number}} product(s) was successfully added to your cart.', '7up-together' ),
					'add_to_cart_fail_single'  => esc_html__( 'One product is out of stock.', '7up-together' ),
					'add_to_cart_fail_plural'  => esc_html__( '{{number}} products were out of stocks.', '7up-together' )
				)
			);
