	...
	// add main/sub classes to li's and links
 	function start_el(  &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
	    $indent = ( $depth > 0 ? str_repeat( "\t", $depth ) : '' ); // code indent
	  	...
	  	if(!empty($content_item)) $content = S7upf_Template::get_vc_pagecontent($content_item);	  	
	    ...
	    if(is_object($args)){
		    ...
		    if(!empty($image))  $item_output .= '<div class="preview-image">
	  		...
	  		if($mega_menu){
	    		...
	    		if($depth == 0) $output .= '<div class="mega-menu" '.s7upf_add_html_attr('width:'.esc_attr($width)).'>'.do_shortcode($content).'</div>';
	    	}	
