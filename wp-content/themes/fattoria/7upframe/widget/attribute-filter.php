...
if(!class_exists('S7upf_Attribute_Filter') && class_exists("woocommerce")){
    class S7upf_Attribute_Filter extends WP_Widget {
        ...
        {
            if(function_exists('s7upf_reg_widget')) s7upf_reg_widget( 'S7upf_Attribute_Filter' );
        }

        function __construct() {
            // Instantiate the parent object
            parent::__construct( false, esc_html__('Attribute Filter','fattoria'),
                array( 'description' => esc_html__( 'Filter product shop page', 'fattoria' ), ));
                                    ...
                                    foreach ($terms as $term){
                                        if(is_object($term)){
                                            ...
                                            if(!empty($color)){
                                                ...
                                                echo sprintf(
                                                    '<a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-color '.$class_white_color.' swatch-%s %s" '.s7upf_add_html_attr('background-color:'.$color).' title="%s" href="%s"><span class="hide">%s</span></a>',
                                                    esc_attr($attribute),
                                                    esc_attr( $term->slug ),
                                                    esc_attr( $term->slug ),
                                                    $selected,
                                ...
                                foreach ($terms as $term) {
                                    if(is_object($term)){
                                        ...
                                        echo    '<li class="'.esc_attr($term->slug).'-inline">
                                                    ...
                                                    <span class="count-cat">'.$term->count.'</span>
                                                </li>';
                                    }
                                }
                            }
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Attribute:' ,'fattoria'); ?></label></br>
                <select id="<?php echo esc_attr($this->get_field_id( 'attribute' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'attribute' )); ?>">
                    ...
                    else echo esc_html__("No any attribute.","fattoria");
                    ?>
                </select>                
            </p>
        <?php
