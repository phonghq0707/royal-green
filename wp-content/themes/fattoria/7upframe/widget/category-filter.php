...
if(!class_exists('S7upf_Category_Fillter') && class_exists("woocommerce"))
{
    class S7upf_Category_Fillter extends WP_Widget {
        ...
        {
            if(function_exists('s7upf_reg_widget')) s7upf_reg_widget( 'S7upf_Category_Fillter' );
        }

        function __construct() {
            // Instantiate the parent object
            parent::__construct( false, esc_html__('Categories Filter','fattoria'),
                    ...
                    echo        '<ul>';                
                        ...
                        foreach ($category as $cat_slug) {
                            ...
                            if(is_object($cat)){
                                ...
                                echo        '<li><a data-cat='.esc_attr($cat->slug).' href="'.esc_url(s7upf_get_filter_url('product_cat',$cat->slug)).'" class="load-shop-ajax '.$active.'"> '.$cat->name.'</a><span class="count-cat">'.$cat->count.'</span></li>';
                            }
                        }
                    echo      '</ul>';
                }      
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Categories:' ,'fattoria'); ?></label></br>
                <?php 
                ...
                else echo esc_html__("No any category.","fattoria");
                ?>
            </p>            
        <?php
        }
