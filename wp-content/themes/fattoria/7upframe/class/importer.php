        ...
        static function _do_import()
        {
            if(isset($_REQUEST['7up_do_import']) and $_REQUEST['7up_do_import'] || isset($_REQUEST['media']))
            {
                ...
                if(!current_user_can('manage_options')){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('You do not have permission to do this','fattoria')));die;
                }
                ...
                if ( !class_exists( 'WP_Importer' ) or ! class_exists( 'WP_Import' ) ){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('Importer Class Was Not Installed','fattoria')));die;
                }
                global $s7upf_config;
                    ...
                    if(!empty($data_option)){
                        ...
                        if(!function_exists('ot_options_id'))
                        {
                            echo json_encode( array(
                                    'status'   =>0,
                                    'messenger'=>"<span class='red'>".esc_html__("Plugin: 7up core must be installed first. Stop working!","fattoria")."</span>",
                                    'next_url' => ''
                                )
                            );
                            die;
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Importing the demo theme options...","fattoria")."<span>".esc_html__("DONE!","fattoria")."</span><br>",
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Theme option contain NULL content. Stop working!","fattoria")."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==2){
                    ...
                    if(!empty($data_object)){
                        ...
                        echo json_encode( array(
                                'status'   =>1,
                                'messenger'=>esc_html__("Importing the demo widgets...","fattoria")." <span>".esc_html__("DONE!","fattoria")."</span>.<br>",
                                'next_url' => admin_url(self::$_import_page."&7up_do_import=1&media={$import_media}&package={$package}&step=" . ($step + 1)),
                            )
                        );
                    }
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Widget option contain NULL content. Stop working!","fattoria")."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==3){
                    ...
                    if(!$file_name){
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("File Not Found. Stop working!","fattoria")."</span>",
                                'next_url' => ''
                            )
                        );
                        die;
                ...
                if($step==4) {
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => esc_html__("Importing menu settings ...","fattoria")." <span>".esc_html__("DONE!","fattoria")."</span><br>",
                            'next_url' => $nexturl,
                        )
                    );
                }
                ...
                if($step == '5'){
                    ...
                    if(!$set_woocommerce_page) {
                        $next_url = '';
                        $messenger = '<span>'.esc_html__("All Done! Have Fun","fattoria").'</span>';
                    }
                    else {
                    ...
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>esc_html__("Setting reading options...","fattoria")." <span>".esc_html__("DONE!","fattoria")."</span><br/>".$messenger,
                            'next_url' => $next_url,
                        )
                    );
                ...
                if($step=='6')
                {
                    ...
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>esc_html__("Setting Woocommerce options...","fattoria")." <span>".esc_html__("DONE!","fattoria")."</span><br/><span>".esc_html__("All Done! Have Fun","fattoria")."</span>",
                            'next_url' => '',
                        )
                    );
                }                
